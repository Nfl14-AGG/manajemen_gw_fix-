...
if (!empty($model_filter) && !empty($warna_filter) && empty($search_term)) {
...
<h1 class="mb-4">Manajemen Stok Kaos</h1>

<div class="card mb-4">
    <div class="card-header"><i class="bi bi-search"></i> Pencarian Barang</div>
    <div class="card-body">
        <form action="kaos.php" method="GET" class="d-flex">
...
<div class="d-flex justify-content-end mb-4">
    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#tambahModal">
        <i class="bi bi-plus-circle-fill me-2"></i>Tambah Kaos Baru
    </button>
</div>
...
<div class="card">
    <div class="card-header"><i class="bi bi-table"></i> Daftar Stok Kaos</div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    ...
                    $sql = "SELECT id_kaos, model, warna, ukuran, stok_awal, stok_akhir FROM kaos " . $sql_where_clause . " ORDER BY model, warna, ukuran";
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#ubahStokModal" data-id="<?= $row['id_kaos'] ?>" data-nama="<?= htmlspecialchars($row['model']) ?>" data-stok="<?= $row['stok_akhir'] ?>" data-aksi="masuk">
                                <i class="bi bi-plus-lg"></i> Masuk
                            </button>
                            <button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#ubahStokModal" data-id="<?= $row['id_kaos'] ?>" data-nama="<?= htmlspecialchars($row['model']) ?>" data-stok="<?= $row['stok_akhir'] ?>" data-aksi="keluar">
                            ...
                            <a href="proses_stok.php?aksi=hapus_item&tipe_item=kaos&id=<?= $row['id_kaos'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Anda yakin ingin menghapus item ini? Stok akan terhapus jika bernilai 0.');">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Tambah Kaos Baru</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form action="proses_stok.php" method="POST">
        <div class="modal-body">
            <input type="hidden" name="tipe_item" value="kaos">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (ubahStokModal) {
        ubahStokModal.addEventListener('show.bs.modal', function (event) {
            ...
            var stok = button.getAttribute('data-stok'); // Ini adalah stok_akhir
