...
<h1 class="mb-4">Manajemen Material Sablon</h1>

<div class="card mb-4">
    <div class="card-header"><i class="bi bi-search"></i> Pencarian Material</div>
    <div class="card-body">
        <form action="material_sablon.php" method="GET" class="d-flex">
...
<div class="card">
    <div class="card-header"><i class="bi bi-table"></i> Daftar Material Sablon</div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    ...
                    $sql = "SELECT id_sablon, jenis_material, warna, stok_awal, stok_akhir, satuan FROM material_sablon " . $sql_where_clause . " ORDER BY jenis_material, warna";
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#ubahStokModal" data-id="<?= $row['id_sablon'] ?>" data-nama="<?= htmlspecialchars($row['jenis_material']) ?>" data-stok="<?= $row['stok_akhir'] ?>" data-aksi="masuk">
                                <i class="bi bi-plus-lg"></i> Masuk
                            </button>
                            <button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#ubahStokModal" data-id="<?= $row['id_sablon'] ?>" data-nama="<?= htmlspecialchars($row['jenis_material']) ?>" data-stok="<?= $row['stok_akhir'] ?>" data-aksi="keluar">
                                <i class="bi bi-dash-lg"></i> Keluar
                            </button>
                            <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editItemModal" 
                                data-id="<?= $row['id_sablon'] ?>" 
                            ...
                            <a href="proses_stok.php?aksi=hapus_item&tipe_item=material_sablon&id=<?= $row['id_sablon'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Anda yakin ingin menghapus item ini? Stok akan terhapus jika bernilai 0.');">
                    ...
                    ?>
                        <tr>
                            <td colspan="7" class="text-center">
                                ...
                                <?php else: ?>
                                    Belum ada data material sablon.
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endif; ?>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Tambah Material Sablon Baru</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form action="proses_stok.php" method="POST">
        <div class="modal-body">
            <input type="hidden" name="tipe_item" value="material_sablon">
            <input type="hidden" name="aksi" value="tambah_item_baru">
            <div class="mb-3">
                <label for="jenis_material" class="form-label">Jenis Material (Contoh: Tinta Plastisol)</label>
            ...
            <div class="mb-3">
                <label for="satuan" class="form-label">Satuan (Contoh: kg, liter)</label>
    ...
    <div class="modal-content">
      ...
      <form action="proses_stok.php" method="POST">
        <div class="modal-body">
            ...
            <input type="hidden" name="id_item" id="edit_id_item_sablon">
            <div class="mb-3">
                <label for="edit_jenis_material_sablon" class="form-label">Jenis Material</label>
                <input type="text" class="form-control" id="edit_jenis_material_sablon" name="jenis_material" required>
            </div>
            <div class="mb-3">
                <label for="edit_warna_sablon" class="form-label">Warna</label>
                <input type="text" class="form-control" id="edit_warna_sablon" name="warna" required>
            </div>
            <div class="mb-3">
                <label for="edit_satuan_sablon" class="form-label">Satuan</label>
                <input type="text" class="form-control" id="edit_satuan_sablon" name="satuan" required>
            </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (editItemModal) {
        editItemModal.addEventListener('show.bs.modal', function (event) {
            ...
            editItemModal.querySelector('#edit_id_item_sablon').value = id;
            editItemModal.querySelector('#edit_jenis_material_sablon').value = jenis_material;
            editItemModal.querySelector('#edit_warna_sablon').value = warna;
            editItemModal.querySelector('#edit_satuan_sablon').value = satuan;
        });
    }
});
</script>
